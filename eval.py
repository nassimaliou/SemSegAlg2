...
def reshape_psp(input):
    ...
    long_size = round(1 * 1024)
    new_h = long_size
    new_w = long_size
    if h > w:
        new_w = round(1024/float(h)*w)
    else:
        new_h = round(1024/float(w)*h)

    image_scale = cv2.resize(image, (new_w, new_h), interpolation=cv2.INTER_LINEAR)
    ori_h, ori_w, _ = image_scale.shape
    pad_h = max(449 - ori_h, 0)
    pad_w = max(449 - ori_w, 0)
    ...
    stride_h = int(np.ceil(449*(2/3)))
    stride_w = int(np.ceil(449*(2/3)))
    grid_h = int(np.ceil(float(new_h-449)/stride_h) + 1)
    grid_w = int(np.ceil(float(new_w-449)/stride_w) + 1)

    for index_h in range(0, grid_h):
        for index_w in range(0, grid_w):
            s_h = index_h * stride_h
            e_h = min(s_h + 449, new_h)
            s_h = e_h - 449
            s_w = index_w * stride_w
            e_w = min(s_w + 449, new_w)
            s_w = e_w - 449
...
if __name__ == '__main__':
    ...
    parser.add_argument('--theta ', default =2000. , type=float)
    ...
    parser.add_argument('--test_list', type=str, default='/home/nassim/work/internship_with_SerVal/Cityscapes/list/fine_val.txt')
    parser.add_argument('--alpha_init ', type=int , default =0.3)
    ...
    if args.data_path is None:
        args.data_path = "/home/nassim/work/internship_with_SerVal/Cityscapes"
    ...
    if args.dataset == 'Cityscapes':
        if args.model == "pt_pspnet":
            ...
            test_transform = transform.Compose([
                transform.Resize((IMAGENET_SL, IMAGENET_SL)),
                transform.ToTensor()
                ])
            test_data = dataset.SemData(split=args.split, data_root=args.data_path, data_list=args.test_list, transform=test_transform)
            test_loader = torch.utils.data.DataLoader(test_data, batch_size=5, shuffle=False, num_workers=2, pin_memory=True)
